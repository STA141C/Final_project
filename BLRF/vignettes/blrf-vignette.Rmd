...
|`implement_check_input()` | Checking if all inputs are valid for function implement_BLRF.x| True or False|
|`predict_check_input()` | Checking if all inputs are valid for function predict_blrf.| True or False|
...
cls_blrf <- blrf(Type~., train_glass_sample, gamma=0.5, b = NULL, s=10, r=200, n_var=5,  core = 4)
## attributes for brlf object
cls_blrf$Trees
cls_blrf$fitted_prob
cls_blrf$fitted_label
...
* Run `predict()` function to make predictions: set confidence = F, set probability = F, return predicted labels for data
```{r, warning=FALSE, results='hide',message=FALSE}
## make predictions on y lable
pred <- predict(cls_blrf, test_glass_sample_x, confidence = F, probability = F)
pred[[1]]
```
